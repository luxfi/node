...
import (
	...
	"github.com/luxfi/node/mesh/kem"
...
func (n *mesh) sequencerID(chainID ids.ID) ids.ID {
...
func (n *mesh) Send(
	...
	var allower network.Allower = &noOpAllower{}
...
func (n *mesh) HealthCheck(context.Context) (interface{}, error) {
	n.peersLock.RLock()
...
func (n *mesh) IngressConnCount() int {
...
func (n *mesh) Connected(nodeID ids.NodeID) {
...
func (n *mesh) AllowConnection(nodeID ids.NodeID) bool {
...
func (n *mesh) Track(claimedIPPorts []*endpoints.ClaimedIPPort) error {
	_, areWeAPrimaryNetworkAValidator := n.config.Validators.GetValidator(constants.PrimaryNetworkID, n.config.MyNodeID)
...
func (n *mesh) Disconnected(nodeID ids.NodeID) {
	n.peersLock.RLock()
...
func (n *mesh) KnownPeers() ([]byte, []byte) {
...
func (n *mesh) ManuallyTrack(nodeID ids.NodeID, endpoint endpoints.Endpoint) {
...
func (n *mesh) track(ip *endpoints.ClaimedIPPort, trackAllNets bool) error {
...
func (n *mesh) getPeers(
	nodeIDs set.Set[ids.NodeID],
	chainID ids.ID,
	allower network.Allower,
) []peer.Peer {
...
func (n *mesh) disconnectedFromConnecting(nodeID ids.NodeID) {
...
func (n *mesh) disconnectedFromConnected(peer peer.Peer, nodeID ids.NodeID) {
...
func (n *mesh) dialEndpointOnly(ip *trackedIP) {
...
// attempts.
func (n *mesh) dial(nodeID ids.NodeID, ip *trackedIP) {
...
func (n *mesh) upgrade(conn net.Conn, upgrader peer.Upgrader, isIngress bool) error {
	direction := "outbound"
	if isIngress {
		direction = "inbound"
	}
...
func (n *mesh) PeerInfo(nodeIDs []ids.NodeID) []peer.Info {
	n.peersLock.RLock()
	defer n.peersLock.RUnlock()
...
func (n *mesh) NodeUptime() (UptimeResult, error) {
...
func (n *mesh) TrackChain(chainID ids.ID) error {
	if chainID == constants.PrimaryNetworkID {
...
// TrackedChains returns the set of chains this node is tracking
func (n *mesh) TrackedChains() set.Set[ids.ID] {
	n.peersLock.RLock()
	defer n.peersLock.RUnlock()
...
func (n *mesh) RegisterBlockchainNetwork(blockchainID, networkID ids.ID) {
	n.peersLock.Lock()
	defer n.peersLock.Unlock()
...
func (n *mesh) RegisterChainIdentity(id peer.ChainIdentity) {
...
func (n *mesh) pullGossipPeerLists() {
	peers := n.samplePeers(
		warp.SendConfig{
			NonValidators: 1,
		},
...
func (n *mesh) getLastReceived() (time.Time, bool) {
...
func (n *mesh) getLastSent() (time.Time, bool) {
