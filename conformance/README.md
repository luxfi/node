...
and the harness cannot say how much of a gap is the missing pass. A chain's
three rows become comparable the day nothing under DECLINED names it — which is
the same gap the `SKIPPED` field already reports to the differential.
...
and every µs/vector in the two tables agrees to within 7%, most within 2%,
`cpp/quantumvm` to within 0.1%. The noise went into the spread, which is what
...
Run it twice and every row of the second lands within 7% of the first, most
within 2% and `cpp/quantumvm` within 0.1% — on a machine whose spreads reach
12000%. That is the case for quoting the fastest run and reading the spread as
weather.

**P and F are the measurement.** They are 423 of the 725 vectors and, with Q
set aside, all but a few percent of every total; they are the two chains that
verify signatures, and all three implementations answer every field of both.
Rust and Go are within 7% on P (60.9 against 64.7 µs a vector) and C++ is 30%
behind them (83.5).
