...
func TestParseBytes(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// Pre-LP-023 BE-encoded fixture: when read by the LE codec
			// the BE-shaped 4-byte slice-length field at offset ~48
			// becomes a huge integer, tripping the max-slice-length
			// gate BEFORE the insufficient-length check. Spec change
			// from ErrInsufficientLength -> ErrMaxSliceLenExceeded.
			...
			expectedErr: pcodecs.ErrMaxSliceLenExceeded,
		},
		{
			name:        "gibberish",
			hex:         "000102030405",
