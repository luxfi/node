...
func TestStateSyncGetOngoingSyncStateSummary(t *testing.T) {
	...
	innerSummaryID := ids.ID{'s', 'u', 'm', 'm', 'a', 'r', 'y', 'I', 'D'}
	innerSummaryHeight := uint64(2022)
	innerSummaryBytes := []byte{'i', 'n', 'n', 'e', 'r'}

	// No ongoing state summary case
	innerVM.GetOngoingSyncStateSummaryF = func(context.Context) ([]byte, error) {
