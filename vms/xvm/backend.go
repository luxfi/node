// Package xvm GPU backend selection — pure-Go shim that probes the
...
//
// Decomplected from xvm_gpu.go / xvm_gpu_nocgo.go: the probe order and
// package-level handle live here. The actual dlopen + dlsym calls live
// in xvm_gpu.go (cgo build) or in the nocgo stub. Both files expose the
// same openGPUBackend signature so this file compiles identically in
// either mode.
//
// If a plugin opens cleanly, the bridge calls route transitions through
// it. If a probe fails (no plugin installed, missing runtime, etc.) the
// bridge runs the canonical pure-Go impl in xvm_gpu_cpu.go. Every GPU
// launcher error makes the caller fall back to the Go path WITHOUT panic
// — strict positive overlay with graceful degradation.
package xvm
...
// path component). The probe layer combines this with searchPaths().
...
//  3. Empty string ("" — let dlopen consult the system search path).
//
...
func searchPaths() []string {
	...
	out = append(out, "/usr/local/lib/lux-gpu", "")
	return out
}
...
// satisfied the probe at init(). Callers MAY check b.IsAvailable() if
// they want to branch on GPU availability; the bridge methods accept a
// nil receiver and fall back to the pure-Go path automatically.
...
// SetActiveGPUBackend replaces the active backend. Used by tests to
// inject a mock or null plugin. Returns the previous backend (caller is
// responsible for closing it). Passing nil clears the slot.
...
// =============================================================================
// AutoBackend — pure-Go probe driver. Walks candidatePlugins() in order
// and returns the first one that opens cleanly. Returns nil if every
// probe fails (no plugin installed, missing runtime, etc.).
// =============================================================================

// AutoBackend probes the canonical plugin search paths in the cuda →
// hip → metal → vulkan → webgpu order and returns the first backend
// that opens AND resolves all four host launchers. Returns nil + the
// last error if no plugin satisfies the probe.
...
// =============================================================================
// init() — runs the canonical probe order at process start. The result
// is stored in the package-level activeBackend slot. Any error is
// silently swallowed (the bridge runs the Go path) so a missing plugin
