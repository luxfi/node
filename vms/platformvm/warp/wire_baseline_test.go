...
func TestWireBaselinePreservedForHybridRename(t *testing.T) {
	...
	// EXACT post-LP-023-phase-7 baseline. These hex strings ARE the
	// on-chain wire format under the ZAP-native LE encoding. Per
	// LP-023, the multi-manager version prefix is uint16 LE and the
	// inner codec body (linear-typed but zapcodec-backed) packs
	// uint32 lengths and uint32 type IDs in LE. Type and field name
	// changes are wire-safe — typeIDs come from registration order
	// and field encodings come from declaration order.
	//
	// To regenerate: run this test, capture the `got` values, paste.
