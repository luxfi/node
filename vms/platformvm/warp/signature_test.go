	...
	tests := []struct {
	...
	}{
		{
			...
			stateF: func(t *testing.T) validators.State {
				return &validatorStateAdapter{
					getNetIDFunc: func(ctx context.Context, chainID ids.ID) (ids.ID, error) {
		...
		},
		{
			...
			stateF: func(t *testing.T) validators.State {
				return &validatorStateAdapter{
					...
					getValidatorSetFunc: func(ctx context.Context, height uint64, netID ids.ID) (map[ids.NodeID]*validators.GetValidatorOutput, error) {
						return nil, errTest
					},
				}
			},
		...
		},
		{
			...
			stateF: func(t *testing.T) validators.State {
				...
				state.EXPECT().GetNetID(gomock.Any(), sourceChainID).Return(subnetID, nil)
