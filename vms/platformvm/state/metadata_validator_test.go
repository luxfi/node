...
func TestParseValidatorMetadata(t *testing.T) {
	...
	tests := []test{
		...
		},
		{
			...
			// preDelegateeRewardSize = 26 bytes (CodecVersionLen + 3*LongLen).
			// LE-encoded by zapcodec-backed MetadataCodec.
			bytes: []byte{
				// codec version v0 (uint16 LE) = {0x00, 0x00}
				0x00, 0x00,
				// UpDuration = 6_000_000 (LE)
				0x80, 0x8d, 0x5b, 0x00, 0x00, 0x00, 0x00, 0x00,
				// LastUpdated = 900_000 (LE)
				0xa0, 0xbb, 0x0d, 0x00, 0x00, 0x00, 0x00, 0x00,
				// PotentialReward = 100_000 (LE)
		...
		},
		{
			...
			// preStakerStartTimeSize = 34 bytes (CodecVersionLen + 4*LongLen).
			// LE-encoded by zapcodec-backed MetadataCodec.
			bytes: []byte{
				// codec version v0 (uint16 LE)
				0x00, 0x00,
				// UpDuration = 6_000_000 (LE)
				0x80, 0x8d, 0x5b, 0x00, 0x00, 0x00, 0x00, 0x00,
				// LastUpdated = 900_000 (LE)
				...
				// PotentialReward = 100_000 (LE)
				0xa0, 0x86, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00,
				// PotentialDelegateeReward = 20_000 (LE)
		...
		},
		{
			...
			// 34-byte preStakerStartTimeSize shape but with v2 prefix.
			// MetadataCodec registers only v0+v1, so Unmarshal returns
			// ErrUnknownVersion before reading payload.
			bytes: []byte{
		...
		{
			name: "short byte len",
			// 32-byte payload: not in {26, 34} so falls through to the
			// default v1+ validatorMetadata branch which expects 42
			// bytes. Underlying zapcodec surfaces ErrInsufficientLength.
			bytes: []byte{
				// codec version v0 (uint16 LE)
				0x00, 0x00,
				// UpDuration (LE)
				...
				// PotentialReward (LE)
				0xa0, 0x86, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00,
				// PotentialDelegateeReward truncated to 6 bytes — short.
