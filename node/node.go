...
type Node struct {
	...
	// consented is the operator's --chains list resolved to genesis chain IDs.
	consented set.Set[ids.ID]
...
func (n *Node) initChainManager(utxoAssetID ids.ID) error {
	...
	// Resolve the operator's consent list once, here, and say per chain what it
	// decided. Enforcement is downstream in authorizeChainActivation from
	// ManagerConfig.NeedConsent/Consented; these lines only report the intent,
	// because an operator whose chain is missing needs to know whether they
	// declined it or the network never had it.
	consented, err := consentedChains(n.Config.GenesisBytes, n.Config.Chains)
	if err != nil {
		return err
	}
	n.consented = consented
	for chainID, name := range consentNames(n.Config.GenesisBytes) {
		if n.consents(chainID) {
			n.Log.Info("chain consented: activation enabled", "name", name, "chainID", chainID)
		} else {
			n.Log.Info("chain in genesis but not in --chains — not tracked even under --track-all-chains", "name", name, "chainID", chainID)
		}
	}

	// The D-Chain ID is still resolved for the ManagerConfig even when D is not
	// critical: its VM is served by the dexd plugin out of PluginDir, so the ID
	// derived from genesis is meaningful whether or not a plugin is installed.
	var dChainID ids.ID
	...
	n.chainManager, err = chains.New(
		&chains.ManagerConfig{
			...
			// NeedConsent/Consented are the operator's chain opt-in, enforced as
			// a necessary activation condition by authorizeChainActivation and
			// composed AND-wise with the entitlement above: a chain the operator
			// did not name in --chains does not activate even under
			// --track-all-chains.
			NeedConsent:                             consentChainsFor(n.Config.GenesisBytes),
			Consented:                               consented,
...
func (n *Node) initVMs() error {
	...
	// Register every VM in the one registry (node/vms.go). P and X were built
	// just above with their runtime dependencies; the rest carry a factory and
	// are linked into this binary, so a chain in genesis starts because the
	// daemon already contains its VM.
	if err := n.registerVMs(); err != nil {
		return err
	}
