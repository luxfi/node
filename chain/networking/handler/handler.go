...
import (
	...
	commontracker "github.com/luxfi/node/chain/engine/common/tracker"
)
...
type handler struct {
	...
	msgFromVMChan   chan core.Message
...
func New(
	...
	h := &handler{
		...
		cn:                cn,
		msgFromVMChan:     make(chan core.Message),
...
func (h *handler) selectStartingGear(ctx context.Context) (core.Engine, error) {
...
func (h *handler) Start(ctx context.Context, recoverPanic bool) {
	...
	h.nf = core.NewNotificationForwarder(h, h.subscription, h.ctx.Log)
...
func (h *handler) Notify(ctx context.Context, msg core.Message) error {
	select {
	case <-ctx.Done():
		return ctx.Err()
	case <-h.closed:
...
func (h *handler) handleSyncMsg(ctx context.Context, msg Message) error {
	...
	if !ok {
		...
		// This can happen if the chain only has a Chain engine and the peer
...
func (h *handler) handleChanMsg(msg message.InboundMessage) error {
	...
	case *message.VMMessage:
		return engine.Notify(context.TODO(), core.Message(msg.Notification))
