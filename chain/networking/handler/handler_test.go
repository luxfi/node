...
func TestHandlerClosesOnError(t *testing.T) {
	...
	subscription := func(context.Context) (core.Message, error) {
...
func TestHandlerDispatchInternal(t *testing.T) {
	...
	notified := make(chan core.Message)
	...
	engine.NotifyF = func(ctx context.Context, msg core.Message) error {
		select {
		case notified <- msg:
			notified <- msg
			return nil
...
func TestDynamicEngineTypeDispatch(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			setup: func(h Handler, b core.BootstrapableEngine, e core.Engine) {
...
func createSubscriber() (core.Subscription, chan<- core.Message) {
