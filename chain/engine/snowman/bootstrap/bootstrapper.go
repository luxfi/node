...
import (
	...
	"github.com/luxfi/node/chain/consensus/snowman/bootstrapper"
	"github.com/luxfi/node/chain/engine/common"
	"github.com/luxfi/node/chain/engine/snowman/block"
	"github.com/luxfi/node/chain/engine/snowman/bootstrap/interval"
...
func New(config Config, onFinished func(ctx context.Context, lastReqID uint32) error) (*Bootstrapper, error) {
	...
	bs.TimeoutRegistrar = core.NewTimeoutScheduler(timeout, config.BootstrapTracker.AllBootstrapped())
...
func (b *Bootstrapper) restartBootstrapping(ctx context.Context) error {
	...
	b.restarted = true
	b.outstandingRequests = bimap.New[core.Request, ids.ID]()
	b.outstandingRequestTimes = make(map[core.Request]time.Time)
	return b.startBootstrapping(ctx)
}
