...
func TestQuantumFinalityEngine(t *testing.T) {
	...
	// Test 4: Submit Q-Chain Corona signature
	t.Run("SubmitQChainCorona", func(t *testing.T) {
		rtSig := &corona.Signature{}
		err := engine.SubmitQChainCorona(block.BlockID, rtSig)
		require.NoError(err)
...
func TestCChainQuantumFinality(t *testing.T) {
	...
	err = engine.SubmitQChainCorona(consensusBlock.BlockID, rtSig)
	require.NoError(err)
