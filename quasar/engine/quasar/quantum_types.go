...
// rt (Corona) package placeholder types until external module is available
type rt struct{}

// Precomp represents precomputed Corona values
type Precomp interface{}

// Share represents a Corona share
type Share []byte

// Cert represents a Corona certificate
type Cert []byte

// Precompute precomputes Corona values from a secret key
func Precompute(sk []byte) (Precomp, error) {
	return nil, nil
}

// QuickSign creates a Corona share
func QuickSign(pre Precomp, msg []byte) (Share, error) {
	return nil, nil
}

// Aggregate aggregates Corona shares into a certificate
