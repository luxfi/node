...
type KeyManager struct {
	...
	// DKG state (for threshold Corona)
	dkgInProgress   bool
...
type EpochKeys struct {
	Epoch           uint32
	GroupPublicKey  []byte // For Corona threshold
...
func NewKeyManager(nodeID ids.NodeID, blsSigner bls.Signer, coronaSK []byte) *KeyManager {
...
// GetCoronaKeyPair returns the current Corona keypair
func (km *KeyManager) GetCoronaKeyPair() ([]byte, []byte) {
	km.mu.RLock()
	defer km.mu.RUnlock()
	return km.coronaSK, km.coronaPK
}
