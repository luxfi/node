...
Quasar is a post-quantum finality layer for the Lux Network that provides dual-certificate finality using both classical BLS signatures and quantum-resistant Corona signatures.
...
- **Dual-Certificate Finality**: Blocks require both BLS and Corona signatures
- **Parallel Aggregation**: BLS and Corona signatures are aggregated in parallel for performance
- **Precomputation**: Corona signatures use precomputed values for <200ms signing
...
- **Sub-second Finality**: Target <1s finality with ~700ms for BLS + ~200ms for Corona
...
- Missing Corona signature detection
...
1. **Dual Signatures**: Both BLS and Corona must succeed for finality
2. **Key Rotation**: Corona keys rotate periodically (default: 24h)
...
- **Corona Addition**: ~200ms (using precomputation)
...
1. **Distributed Key Generation (DKG)**: Full implementation for threshold Corona
