...
**Quasar** is Lux Network's dual-certificate consensus engine that requires both BLS and Corona certificates for every block to achieve finality. This revolutionary approach provides immediate post-quantum security while maintaining sub-second performance through parallel verification and precomputation.

### Key Innovations

1. **Dual-Certificate Finality**: Every block requires both BLS and Corona certificates
   ```go
...
2. **Unified RT Key**: One Corona key per validator secures all chains (Q, C, X, M)
...
- **After**: Nova → Corona PQ → Done (2 steps)

### 2. True 2-Round Finality
```
T+0:   Transaction enters Nova DAG
T+1:   Nova builds confidence (existing k-sampling)
T+2:   Corona Phase I (propose frontier)
T+3:   Corona Phase II (commit frontier)
...
Because Nova's confidence d(T) is monotonic (only increases), Corona's lattice structure guarantees:
...
type QBlockReference struct {
    ...
    Certificate  []byte    // Corona certificate (optional)
}
...
4. **Quasar**: Corona PQ focuses the energy into a tight beam
...
### Why Corona Works on Top of Nova

Nova provides three critical properties that make Corona's 2-phase protocol sufficient:
...
These properties mean Corona doesn't need multiple rounds—the underlying Nova has already done the hard work of achieving agreement.

### Lattice Structure

The Corona lattice leverages Nova's DAG structure:
```
         Q[n+1]
        /  |  \
       /   |   \
...
- Corona preserves this safety (doesn't weaken Nova)
- 2-phase commit adds deterministic finality on top

#### Quantum Security  
- Corona signatures are lattice-based (post-quantum)
...
2. A Q-block includes it (Corona Phase II)
...
func (p *PrecomputePool) Run() {
    ...
    for i := 0; i < p.workers; i++ {
        go func() {
            for {
                share := corona.Precompute(p.rtKey)
...
func (a *RTAggregator) CollectAndAggregate(myShare *RTShare) ([]byte, error) {
    ...
    return corona.Aggregate(shares), nil
}
```
...
### X-Chain: Native Corona UTXOs

X-Chain supports quantum-secure outputs natively:

```go
// New UTXO type for quantum security
type CoronaOutput struct {
    OutputType  string  // "CORONA"
    PublicKey   []byte  // Corona public key
    Amount      uint64
    Locktime    uint64
}

// Spending requires Corona signature
type CoronaInput struct {
    OutputID    ids.ID
    Signature   []byte  // ~1.8KB Corona signature
}
...
func (m *MChain) ValidateMPCOp(op MPCOperation) error {
    ...
    if len(op.RTSignature) > 0 {
        if !corona.Verify(op.RTSignature, op.Hash()) {
...
- Compress Corona certificates with BLS
- Deduplicate cross-chain data

### 3. Hardware Acceleration
- GPU acceleration for lattice operations
- FPGA Corona signature verification
...
Quasar represents a fundamental breakthrough in consensus design. By requiring both BLS and Corona certificates for every block, we achieve the impossible: consensus that is simultaneously classical AND quantum secure, without sacrificing performance.
