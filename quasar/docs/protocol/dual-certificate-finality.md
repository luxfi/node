# Dual-Certificate Finality: BLS + Corona Integration

## Overview

Lux Network implements **dual-certificate finality**, requiring both BLS and Corona certificates for every block to be considered final. This architecture provides immediate post-quantum security while maintaining sub-second finality.
...
| **Validator-RT** | **corona-lattice** | **PQ finality shares (P, X, M chains)** | **`$HOME/.lux/rt.key`** |
| Wallet (EVM) | secp256k1 or Lamport XMSS | User tx signatures | In wallet |
| Wallet (X-Chain) | secp256k1 or Corona pubkey | UTXO locking | In wallet |
...
// Spend requires Corona signature (~1.8 KB)
func (o *PQTOutput) Verify(sig []byte) error {
    return corona.Verify(o.PublicKey, sig)
}
```
...
lux-wallet send --pq-sign      # Sign with Corona
```
...
func (q *QuasarService) Run() {
    ...
    q.OnPropose(func(block *Block) {
        ...
        // Create certificate
        rtCert := corona.Aggregate(shares)
