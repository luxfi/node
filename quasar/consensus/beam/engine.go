...
func NewEngine(
	...
	if config.QuasarEnabled {
		quasarConfig := QuasarConfig{
			Threshold:     config.CoronaThreshold,
			QuasarTimeout: config.QuasarTimeout,
			Validators:    ctx.ValidatorState,
		}
		e.quasar, err = NewQuasar(ctx.NodeID, ctx.CoronaSK, quasarConfig)
		if err != nil {
...
// addDualCertificates adds BLS and Corona certificates to a block
func (e *Engine) addDualCertificates(ctx context.Context, block quasar.Block) error {
	...
	// Get Corona certificate with timeout
	rtCert, err := e.getCoronaCertificate(ctx, block)
	if err != nil {
		// Slash proposer for missing RT certificate
		e.slashChannel <- SlashEvent{
			ProposerID: e.ctx.NodeID,
			Height:     block.Height(),
			Reason:     "Quasar timeout: missing Corona certificate",
			Timestamp:  time.Now(),
		}
		return fmt.Errorf("failed to get Corona certificate: %w", err)
	}
...
// getCoronaCertificate gets a Corona certificate for a block
func (e *Engine) getCoronaCertificate(ctx context.Context, block quasar.Block) ([]byte, error) {
	if e.quasar == nil {
