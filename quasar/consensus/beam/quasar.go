...
// PrecompPool manages precomputed Corona data
...
func NewQuasar(nodeID ids.NodeID, rtSK []byte, config QuasarConfig) (*Quasar, error) {
	if len(rtSK) == 0 {
		return nil, errors.New("empty Corona secret key")
	}
...
// OnRTShare handles incoming Corona shares
...
func (q *Quasar) aggregateShares(height uint64) {
	...
	// Convert to corona.Share type
	rtShares := make([]corona.Share, len(shares))
	for i, share := range shares {
		rtShares[i] = corona.Share(share)
	}
...
// QuickSign creates a Corona share using precomputed data
func (q *Quasar) QuickSign(msg []byte) ([]byte, error) {
	...
	share, err := corona.QuickSign(pre, hash[:])
	if err != nil {
		return nil, err
	}
...
// sign creates a Corona share without precomputation
func (q *Quasar) sign(msg []byte) ([]byte, error) {
	hash := sha256.Sum256(msg)
	share, err := corona.Sign(q.rtSK, hash[:])
	if err != nil {
		return nil, err
	}
	return share, nil
...
func VerifyDualCertificates(block *Block, blsPK []byte, rtPK []byte) error {
	...
	blockHash := block.ID()
	if !corona.Verify(rtPK, blockHash[:], block.Certs.RTCert) {
		return errors.New("Corona verification failed")
	}
