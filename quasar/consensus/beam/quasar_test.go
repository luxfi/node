...
// Mock Corona implementation for testing
type MockCorona struct{}

func (m *MockCorona) KeyGen(seed []byte) ([]byte, []byte, error) {
	sk := make([]byte, 64)
	pk := make([]byte, 32)
	copy(sk, seed)
	copy(pk, seed[:32])
...
func (m *MockCorona) Precompute(sk []byte) ([]byte, error) {
	pre := make([]byte, 40960)
	rand.Read(pre)
	return pre, nil
}

func (m *MockCorona) QuickSign(pre []byte, msg []byte) ([]byte, error) {
	share := make([]byte, 430)
	copy(share[:32], msg)
	return share, nil
}

func (m *MockCorona) VerifyShare(pk []byte, msg []byte, share []byte) bool {
	return len(share) == 430
}

func (m *MockCorona) Aggregate(shares [][]byte) ([]byte, error) {
	if len(shares) == 0 {
...
func (m *MockCorona) Verify(pk []byte, msg []byte, cert []byte) bool {
