...
luxd-go:
	...
	@test -x $(BIN)/luxd-go
	@leaked="$$(GOWORK=off go list -deps ./cmd/luxd 2>/dev/null | grep -E 'ava-labs' || true)"; \
	if [ -n "$$leaked" ]; then echo "FAIL: ava-labs in the closure:" >&2; echo "$$leaked" >&2; exit 1; fi
...
chains-build:
	...
	cmake -S $(ROOT)/chains/cpp/oraclevm -B $(ROOT)/chains/cpp/oraclevm/build -DCMAKE_BUILD_TYPE=Release
