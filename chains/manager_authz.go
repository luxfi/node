...
func (m *manager) authorizeChainActivation(chainID ids.ID) (authorized bool, ready bool) {
	...
	// Consent is a NECESSARY condition that composes AND-wise with the
	// entitlement below, and it is checked before the unrestricted
	// short-circuit so it is required either way. A node whose operator did
	// not name the chain declines it even under --track-all-chains: this is
	// the activation authority, not the platformvm tracking set.
	//
	// One rule for every chain that needs consent. It was a boolean special
	// -cased to the D-Chain, which answered the question for one chain and
	// left the next one to add a second flag, a second field and a second
	// branch that could disagree with the first.
	if m.NeedConsent.Contains(chainID) && !m.Consented.Contains(chainID) {
		m.Log.Info("chain activation declined: this chain is not in --chains",
			log.String("name", m.ConsentNames[chainID]),
