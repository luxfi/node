...
std::optional<zap::Object> root_of(zap::Message& msg, ByteView value, std::string* err) {
    auto m = zap::Message::parse(value);
    if (!m) {
        if (err != nullptr) *err = std::string(zap::describe(m.error()));
        return std::nullopt;
    }
    msg = *m;
    return msg.root();
...
Id id_at(const zap::Object& o, int off) { return id_from(o.bytes_fixed(off, int(kIdLen))); }
NodeId node_id_at(const zap::Object& o, int off) {
    return node_id_from(o.bytes_fixed(off, int(kNodeIdLen)));
}
