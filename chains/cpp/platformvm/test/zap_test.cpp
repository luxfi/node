...
//
// zap_test.cpp — the wire-format suite, ported from Go zap_test.go
...
using namespace lux::platformvm::zap;
...
TEST(ByteList) {
    ...
    const auto msg = Message::parse(sp(data));
    REQUIRE(msg.has_value());
    REQUIRE(as_string(msg->root().list(0).raw()) == "hello");
}
...
TEST(UncappedListLength) {
    ...
    REQUIRE(list.size() != static_cast<int>(0xFFFFFFFF));
    REQUIRE(static_cast<std::size_t>(list.size()) <= buf.size());
}
...
TEST(ListStrideTighterClamp) {
    ...
    REQUIRE_EQ_NUM(100, msg->root().list(0).size());
...
TEST(BackwardListPointer) {
    ...
    const auto msg = Message::parse(sp(buf));
    REQUIRE(msg.has_value());
    REQUIRE_EQ_NUM(0, msg->root().list(0).size());
}
...
TEST(BuilderEmitsV2) {
    ...
    ob.finish_as_root();
    const auto buf = b.finish();
    REQUIRE_EQ_NUM(kVersion2, load_u16(buf.data() + 4));
    const auto msg = Message::parse(sp(buf));
    REQUIRE(msg.has_value());
