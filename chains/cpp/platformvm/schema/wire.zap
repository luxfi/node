...
# The P-chain's wire, as a schema.
#
# These offsets are the format. They were stated by hand in four places —
# txs.hpp, txs_wire.hpp, txs_wire.cpp and block.hpp — and are stated here
# once instead, so the readers and the builders come out of zapgen rather
# than out of a person, and the C++ chain and the Rust chain and the Go
# reference stop being three chances to mistype the same number.
#
# Every spending transaction opens with the same eight fields — which network,
# which chain, what is spent, what is made — and then says its own thing after
# them. Each kind repeats those eight rather than naming a shared one, because
# this file is a table of consensus offsets: what a kind is, read straight
# down, with nothing to resolve first. The kind byte at 0 says which table.
#
# Fields are written in the order they are declared, which is the order of
# their offsets, which is the order of the bytes.

package lux.platformvm.wire
...
# that is what lets a transaction be read without walking it. The record
# reserves 72 bytes where its fields end at 68 — the four that make the stride
# eight-aligned — so the width is stated rather than inferred.
struct Out @72 {
    Asset     id32 @0
    StakeLock u64  @32
    Amount    u64  @40
...
# One input, inline in an input list. Its signature indices are a run in the
# transaction-wide index array. Same reserved tail as Out.
struct In @96 {
    TxID      id32 @0
    Index     u32  @32
    Asset     id32 @36
...
# A validator joining the primary network, with what it stakes and where its
# reward goes. The four validator fields at 77 are the 44-byte inline record
# every staking kind opens its own section with.
struct AddValidator {
    Kind             u8         @0
    NetworkID        u32        @1
    BlockchainID     id32       @5
...
# aggregated under and two reward owners. The signer is a 145-byte inline
# record: one byte saying whether there is one, then the key and its proof.
...
# ---- what state and signatures are stored as -----------------------------

# An owner set standing alone, rather than inline in a transaction. This is
# the shape a chain owner is kept in.
struct Owner {
    Threshold u32        @0
    Locktime  u64        @4
    Addrs     list<addr> @12
...
# credential naming a run in one shared signature array. A claimed range that
# is not inside the array it names is a refusal, not a clamp — but that is a
# judgement about meaning, and it is made by the reader, not here.
...
# ---- the fx envelopes an output is ordered by ------------------------------
#
# An output's canonical bytes are its SORT KEY, so they are consensus: two
# implementations that build them differently disagree about which
# transactions are well-formed. The two-byte (TypeKind, ShapeKind) pair each
# travels behind sits OUTSIDE this message — it says which fx family and
# which shape the bytes are, which is a question about the chain rather than
# about the payload.
...
# A transfer output held until a time, carrying the whole envelope of the
# output it releases — discriminator and all, so it is a byte run here.
struct Locked {
    Locktime u64   @0
    Output   bytes @8
}

# One unspent output standing alone, behind its own envelope: where it came
# from, and the whole output it is — discriminator and all.
struct Utxo {
    TxID   id32  @0
    Index  u32   @32
    Asset  id32  @36
