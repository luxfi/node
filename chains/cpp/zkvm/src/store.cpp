...
// same shape every variable section on this chain uses.
...
Bytes to_bytes(ByteView v) { return bytes_of(v); }
...
Bytes encode_batch(const ByteMap<std::optional<Bytes>>& batch) {
    ...
    const int key_lens_off = wire::write_u32_list(b, key_lens);
    const int val_lens_off = wire::write_u32_list(b, val_lens);
...
Result<ByteMap<std::optional<Bytes>>> decode_batch(ByteView record) {
    ...
    const auto val_lens = wire::read_u32_list(root, kOffValLens);
    ...
    ByteMap<std::optional<Bytes>> out;
...
Result<std::optional<Bytes>> Memory::get(ByteView key) const {
    auto it = rows_.find(key);
    if (it == rows_.end()) return std::optional<Bytes>{};
    return std::optional<Bytes>(it->second);
}

Result<void> Memory::put(ByteView key, ByteView value) {
    rows_[to_bytes(key)] = to_bytes(value);
    return {};
}

Result<void> Memory::erase(ByteView key) {
    rows_.erase(to_bytes(key));
    return {};
}
...
Result<std::optional<Bytes>> View::get(ByteView key) const {
    auto it = staged_.find(key);
    if (it != staged_.end()) {
        if (!it->second.has_value()) return std::optional<Bytes>{};
        return std::optional<Bytes>(*it->second);
    }
    return base_->get(key);
}

Result<void> View::put(ByteView key, ByteView value) {
    staged_[to_bytes(key)] = to_bytes(value);
    return {};
}

Result<void> View::erase(ByteView key) {
    staged_[to_bytes(key)] = std::nullopt;
    return {};
}

Result<void> View::each(ByteView prefix,
                        const std::function<bool(ByteView, ByteView)>& f) const {
    // The two layers are merged in key order so a caller sees ONE ascending
    // sequence: staged rows shadow committed ones, and a staged erase hides the
    // committed row rather than letting the scan resurrect it.
    ByteMap<Bytes> merged;
    auto collect = base_->each(prefix, [&](ByteView k, ByteView v) {
        merged[to_bytes(k)] = to_bytes(v);
        return true;
    });
    ...
    for (const auto& [k, v] : merged) {
        if (!f(view(k), view(v))) return {};
    }
    return {};
}

Result<void> View::commit() {
    for (const auto& [key, val] : staged_) {
        if (val.has_value()) {
            if (auto r = base_->put(view(key), view(*val)); !r) return r;
        } else {
            if (auto r = base_->erase(view(key)); !r) return r;
        }
    }
    staged_.clear();
    return base_->commit();
...
Result<std::unique_ptr<File>> File::open(const std::string& path) {
    ...
    if (fd < 0)
        return std::unexpected(std::string(kErrOpen) + " " + path + ": " + std::strerror(errno));
...
Result<void> File::replay() {
    ...
    if (end < 0)
        return std::unexpected(std::string(kErrOpen) + " " + path_ + ": " + std::strerror(errno));
...
Result<std::optional<Bytes>> File::get(ByteView key) const {
    auto it = rows_.find(key);
    if (it == rows_.end()) return std::optional<Bytes>{};
    return std::optional<Bytes>(it->second);
}

Result<void> File::put(ByteView key, ByteView value) {
...
Result<void> File::erase(ByteView key) {
    Bytes k = to_bytes(key);
    auto it = rows_.find(k);
    if (it != rows_.end()) {
        live_bytes_ -= it->first.size() + it->second.size();
...
Result<void> File::each(ByteView prefix,
                        const std::function<bool(ByteView, ByteView)>& f) const {
    for (auto it = rows_.lower_bound(prefix); it != rows_.end(); ++it) {
        if (!has_prefix(it->first, prefix)) break;
        if (!f(view(it->first), view(it->second))) return {};
...
Result<void> File::append(const Bytes& record) {
    ...
    if (::fsync(fd_) != 0)
        return std::unexpected(std::string(kErrWrite) + " " + path_ + ": " + std::strerror(errno));
...
Result<void> File::compact() {
    ...
    ByteMap<std::optional<Bytes>> all;
    ...
    if (fd < 0)
        return std::unexpected(std::string(kErrWrite) + " " + tmp + ": " + std::strerror(errno));
    std::size_t written = 0;
    while (written < record.size()) {
