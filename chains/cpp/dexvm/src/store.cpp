...
Result<std::map<Bytes, std::optional<Bytes>>> decode_batch(ByteView record) {
    ...
    if (key_lens.size() != n || val_lens.size() != n)
        return fail(std::string(kErrCorruptRecord) + ": row count mismatch");
    ...
    for (std::size_t i = 0; i < n; ++i) {
        ...
        if (k_pos + k_len > key_blob.size() || v_pos + v_len > val_blob.size())
            return fail(std::string(kErrCorruptRecord) + ": row " + std::to_string(i) +
                            " out of bounds");
...
Result<std::unique_ptr<File>> File::open(const std::string& path) {
    ...
    if (fd < 0)
        return fail(std::string(kErrOpen) + " " + path + ": " +
                               std::strerror(errno));
...
Result<void> File::replay() {
    ...
    if (end < 0)
        return fail(std::string(kErrOpen) + " " + path_ + ": " +
                               std::strerror(errno));
    ...
    if (cursor != buf.size()) {
        if (::ftruncate(fd_, off_t(cursor)) != 0)
            return fail(std::string(kErrWrite) + " " + path_ + ": " +
                                   std::strerror(errno));
    }
...
Result<void> File::compact() {
    ...
    if (fd < 0)
        return fail(std::string(kErrWrite) + " " + tmp + ": " +
                               std::strerror(errno));
    std::size_t written = 0;
    while (written < record.size()) {
        ...
        if (n < 0) {
            ...
            ::close(fd);
            ::unlink(tmp.c_str());
            return fail(std::string(kErrWrite) + " " + tmp + ": " + why);
        }
        written += std::size_t(n);
    }
    if (::fsync(fd) != 0) {
    ...
    if (::rename(tmp.c_str(), path_.c_str()) != 0) {
        const std::string why = std::strerror(errno);
        ::unlink(tmp.c_str());
        return fail(std::string(kErrWrite) + " " + path_ + ": " + why);
    }
