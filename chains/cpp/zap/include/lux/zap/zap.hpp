...
// Lux chains use. Ported field-for-field from the Go definition (luxfi/zap
...
// nothing; the buffer must outlive them. That is the point of the format, and
// it is why every transaction type caches its own wire bytes.
//
// ONE HOME. This header is chain-neutral on purpose: the codec is the same
// codec for every chain, so it is written down once here rather than vendored
// per chain. The four in-tree copies under chains/cpp/{platformvm,xvm,
// quantumvm,zkvm}/include predate it and differ from it only by namespace;
// they fold into this one as each of those ports next moves.
...
public:
    ...
    // Go SetInt64: the same eight bytes, read back as two's complement.
    void set_i64(int off, std::int64_t v) { set_u64(off, std::uint64_t(v)); }
...
public:
    ...
    // Go Int64: u64 reinterpreted as two's complement.
    std::int64_t i64(int off) const { return std::int64_t(u64(off)); }
