...
void Vm::reject_block(const Id& blk_id) {
    ...
    for (const auto& tx : (*blk)->transactions) {
        ...
        // hold go back into the pool and the ones that no longer do are
        // remembered as dropped, which is exactly what Go does here.
        if (auto r = verify_tx(*tx); !r) {
            pool_.mark_dropped(tx->id(), r.error());
            continue;
        }
        if (auto r = pool_.add(tx); !r) pool_.mark_dropped(tx->id(), r.error());
    }
}
