...
//! ML-DSA identity and an ML-KEM session, and both come from `libluxcrypto` by
//! way of `lux-pq`. `cargo:rustc-link-arg` reaches only the binaries of the
//! crate that emitted it, so the rpath `lux-node` set for its own binaries does
//! not reach this crate's test binaries — and without this they link and then
//! cannot load.
//!
