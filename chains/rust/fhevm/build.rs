...
//!
//! This chain authenticates a payer with ML-DSA-65, which comes from
//! `libluxcrypto` by way of `lux-pq`. `cargo:rustc-link-arg` reaches only the
//! binaries of the crate that emitted it, so the rpath `lux-pq`'s own ABI crate
//! set does not reach this crate's binaries — the conformance evaluator and the
//! test binaries link and then cannot load without this.
//!
