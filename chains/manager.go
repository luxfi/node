...
func (m *manager) createChain(chainParams ChainParameters) {
	...
	// Note: buildChain builds all chain's relevant objects (notably engine and
	// handler) AND starts the consensus engine's background loops with a LIFETIME
	// context (see the finality-wedge note at the Start call inside buildChain).
	// The engine's loops are event-driven and do not emit routed outbound messages
	// merely on start, so starting them here (before Router.AddChain below) does not
	// drop messages; the handler's inbound message servicing only begins once the
	// chain is registered in the router (Router.AddChain, further down).
	...
	// The consensus engine is already started inside buildChain (with a LIFETIME
	// context — see the finality-wedge note there). It is NOT re-started here:
	// engine/chain Start() is idempotent (returns ErrAlreadyStarted), and the old
	// second Start at this site passed a 30s-TIMEOUT context whose deferred cancel
	// was a latent re-wedge — if buildChain's start were ever removed, that timeout
	// would become the engine's lifetime context and kill the vote/re-poll loops
	// ~30s in (the original freeze). One start site only (lifetime ctx, in
	// buildChain). The second Start arg was ignored by the engine regardless.
	if chain.Engine != nil {
