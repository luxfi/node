...
type ManagerConfig struct {
	...
	// Derived in node.go from the single VMs registry; see
	// authorizeChainActivation.
	RestrictedChains set.Set[ids.ID]
	// NeedConsent are the chains an operator must name before this node runs
	// them. Not a security control — anyone may name a chain — it records that
	// running the chain is a decision rather than a default, because the chain
	// costs something validating the primary network did not ask for:
	// co-location with a matcher, an HSM, a GPU, a custody role.
	//
	// Consented are the chains the operator did name (--chains). A chain in
	// NeedConsent and absent from Consented is declined even when
	// --track-all-chains queued it: the activation authority is
	// authorizeChainActivation, not the platformvm tracking set.
	//
	// Both are derived in node.go from the single VMs registry. They compose
	// AND-wise with RestrictedChains where a chain is both.
	NeedConsent set.Set[ids.ID]
	Consented   set.Set[ids.ID]
	// ConsentNames names each consent-requiring chain, so a decline can say
	// which chain the operator did not name rather than printing an id they
	// cannot act on.
	ConsentNames map[ids.ID]string
