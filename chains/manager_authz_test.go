...
// consentManager is the shape node.go produces for a chain that is BOTH
// consent-requiring and restricted — the D-Chain today — with consent GIVEN so
// these tests exercise the entitlement half.
func consentManager(t *testing.T, c *committee) (*manager, ids.NodeID, ids.ID) {
	t.Helper()
	nodeID := ids.GenerateTestNodeID()
	mChainID := ids.GenerateTestID()
	chainID := ids.GenerateTestID()
	m := newManager(nodeID, mChainID, set.Of(chainID), nil, c)
	m.NeedConsent = set.Of(chainID)
	m.Consented = set.Of(chainID)
	return m, nodeID, chainID
}
...
func TestOwnershipAuthorizes(t *testing.T) {
	c := newCommittee(t)
	m, nodeID, dChainID := consentManager(t, c)
...
func TestAbsenceRefuses(t *testing.T) {
	t.Run("no attestation on record", func(t *testing.T) {
		c := newCommittee(t)
		m, _, dChainID := consentManager(t, c)
...
// TestConsentAloneIsNotSufficient is the point of the whole check. Naming a
// chain in --chains states what the operator WANTS; the attestation states what
// they are ENTITLED to. Neither alone activates a chain that needs both.
func TestConsentAloneIsNotSufficient(t *testing.T) {
	t.Run("consented, no entitlement => refused", func(t *testing.T) {
		c := newCommittee(t)
		m, _, chainID := consentManager(t, c)

		authorized, ready := m.authorizeChainActivation(chainID)
		require.True(t, ready)
		require.True(t, m.Consented.Contains(chainID))
		require.False(t, authorized,
			"naming a chain in --chains must NOT be sufficient on its own")
	})

	t.Run("not consented, entitled => refused", func(t *testing.T) {
		c := newCommittee(t)
		m, nodeID, chainID := consentManager(t, c)
		m.Consented = nil
		c.attest(claimFor(nodeID, 7))

		authorized, ready := m.authorizeChainActivation(chainID)
		require.True(t, ready)
		require.False(t, authorized, "consent remains necessary even for an entitled node")
	})

	t.Run("consented and entitled => activated", func(t *testing.T) {
		c := newCommittee(t)
		m, nodeID, chainID := consentManager(t, c)
		c.attest(claimFor(nodeID, 7))
	...
	t.Run("consent is per chain, not global", func(t *testing.T) {
		c := newCommittee(t)
		nodeID := ids.GenerateTestNodeID()
		mChainID := ids.GenerateTestID()
		other := ids.GenerateTestID()
		m := newManager(nodeID, mChainID, set.Of(other), nil, c)
		// A DIFFERENT chain needs consent and did not get it. `other` needs
		// none, so it must be untouched: withholding one chain must never
		// withhold a second.
		m.NeedConsent = set.Of(ids.GenerateTestID())
		m.Consented = nil
		c.attest(claimFor(nodeID, 7))

		authorized, ready := m.authorizeChainActivation(other)
		require.True(t, ready)
		require.True(t, authorized, "consent must gate ONLY the chains that require it")
	})

	t.Run("a consent-only chain activates on consent alone", func(t *testing.T) {
		// B and M need consent but are not restricted. Nothing else may be
		// asked of them, or the operator has a flag that silently does nothing.
		chainID := ids.GenerateTestID()
		m := newManager(ids.GenerateTestNodeID(), ids.GenerateTestID(), nil, nil, newCommittee(t))
		m.NeedConsent = set.Of(chainID)
		m.Consented = set.Of(chainID)
		...
		authorized, ready = m.authorizeChainActivation(chainID)
		require.True(t, ready)
		require.False(t, authorized, "an unnamed consent-requiring chain must decline")
	})
}
...
func TestUnconfiguredRefusesRatherThanGuessing(t *testing.T) {
	t.Run("no M-Chain in genesis", func(t *testing.T) {
		...
		m.NeedConsent = set.Of(dChainID)
		m.Consented = set.Of(dChainID)
