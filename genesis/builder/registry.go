...
// VMByName resolves an operator-typed chain name — a letter ("D"), an alias
// ("dex"), or the VM's own name ("dexvm") — to its VMID. Matching ignores case
// so an operator need not remember that the letters are capitals and the
// aliases are not. Reports false for a name no chain answers to, so a typo is
// refused rather than silently electing nothing.
func VMByName(name string) (ids.ID, bool) {
	want := strings.ToLower(strings.TrimSpace(name))
	if want == "" {
		return ids.Empty, false
	}
	for _, s := range Registry {
		if strings.ToLower(s.Letter) == want {
			return s.VMID, true
		}
