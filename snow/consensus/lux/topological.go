...
import (
	...
	"github.com/luxdefi/node/snow/consensus/metrics"
...
// Topological performs the lux algorithm by utilizing a topological sort
...
func (ta *Topological) HealthCheck(ctx context.Context) (interface{}, error) {
	...
	if !healthy {
		...
		return details, fmt.Errorf("lux consensus is not healthy reason: %s", strings.Join(errorReasons, ", "))
	}
	return details, nil
}
