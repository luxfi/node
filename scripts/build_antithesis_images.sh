...
function build_builder_image_for_node {
  ...
  build_antithesis_builder_image "${GO_VERSION}" "antithesis-node-builder:${IMAGE_TAG}" "${AVALANCHE_PATH}" "${AVALANCHE_PATH}"
}
...
if [[ "${TEST_SETUP}" == "node" ]]; then
  ...
  gen_antithesis_compose_config "${IMAGE_TAG}" "${AVALANCHE_PATH}/tests/antithesis/node/gencomposeconfig" \
                                "${AVALANCHE_PATH}/build/antithesis/node"

  build_antithesis_images_for_node "${TEST_SETUP}" "${IMAGE_PREFIX}" "${AVALANCHE_PATH}/Dockerfile" "${NODE_ONLY:-}"
else
  build_builder_image_for_node
  ...
  NODE_ONLY=1
  build_antithesis_images_for_node node "" "${AVALANCHE_PATH}/Dockerfile" "${NODE_ONLY}"
  ...
  gen_antithesis_compose_config "${IMAGE_TAG}" "${AVALANCHE_PATH}/tests/antithesis/xsvm/gencomposeconfig" \
                                ...
                                "LUXD_PATH=${AVALANCHE_PATH}/build/node AVAGO_PLUGIN_DIR=${AVALANCHE_PATH}/build/plugins"

  build_antithesis_images_for_node "${TEST_SETUP}" "${IMAGE_PREFIX}" "${AVALANCHE_PATH}/vms/example/xsvm/Dockerfile"
fi
