...
# intended to be reusable by repos other than luxd so almost all inputs are accepted as parameters
...
function build_antithesis_builder_image {
  ...
  local base_dockerfile="${luxd_path}/tests/antithesis/Dockerfile"
...
function build_antithesis_images {
 ...
 --build-arg BUILDER_WORKDIR=${builder_workdir}"
  if [[ -n "${luxd_commit}" ]]; then
    docker_cmd="${docker_cmd} --build-arg LUXD_COMMIT=${luxd_commit}"
  fi

  # By default the node image is intended to be local-only.
  LUXD_IMAGE="antithesis-luxd-node:${node_image_tag}"
  ...
  if [[ -n "${image_prefix}" && -z "${node_only}" ]]; then
    ...
    LUXD_IMAGE="${image_prefix}/${LUXD_IMAGE}"
  fi

  # Ensure the correct node image name is configured
  docker_cmd="${docker_cmd} --build-arg LUXD_IMAGE=${LUXD_IMAGE}"

  # Build node image first to allow the workload image to use it.
  ${docker_cmd} -t "${node_image_name}" -f "${node_dockerfile}" "${target_path}"

  if [[ -n "${node_only}" ]]; then
    # Skip building the config and workload images. Supports building the luxd node image as the
