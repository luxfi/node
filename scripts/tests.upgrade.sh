...
# e.g.,
# ./scripts/build.sh
# ./scripts/tests.upgrade.sh 1.7.16 ./build/luxd
...
GOOS=$(go env GOOS)
DOWNLOAD_URL=https://github.com/luxdefi/luxd/releases/download/v${VERSION}/luxd-linux-${GOARCH}-v${VERSION}.tar.gz
DOWNLOAD_PATH=/tmp/luxd.tar.gz
if [[ ${GOOS} == "darwin" ]]; then
  DOWNLOAD_URL=https://github.com/luxdefi/luxd/releases/download/v${VERSION}/luxd-macos-v${VERSION}.zip
...
DOWNLOAD_URL=https://github.com/luxdefi/avalanche-network-runner/releases/download/v${NETWORK_RUNNER_VERSION}/avalanche-network-runner_${NETWORK_RUNNER_VERSION}_linux_amd64.tar.gz
if [[ ${GOOS} == "darwin" ]]; then
  DOWNLOAD_URL=https://github.com/luxdefi/avalanche-network-runner/releases/download/v${NETWORK_RUNNER_VERSION}/avalanche-network-runner_${NETWORK_RUNNER_VERSION}_darwin_amd64.tar.gz
fi
...
--network-runner-luxd-path=/tmp/luxd-v${VERSION}/luxd \
--network-runner-luxd-path-to-upgrade=${NEW_BINARY} \
--network-runner-luxd-log-level="WARN" || EXIT_CODE=$?
