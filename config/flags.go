...
func addNodeFlags(fs *pflag.FlagSet) {
	...
	fs.StringSlice(ChainsKey, nil, "Chains this node consents to run, by letter or alias (e.g. --chains=D,B,M). Some primary-network chains cost an operator something validating the primary network did not ask for — co-location with a matcher, an HSM, a GPU, a custody role — so the node runs them only when named here, even under --track-all-chains. Naming a chain says what the operator WANTS; where a chain is also restricted, activation still requires an M-Chain ownership attestation naming this node, which is what they are ENTITLED to")
	...
	fs.String(StateSyncIDsKey, "", "Comma separated list of state sync peer ids to connect to, each in NodeID-<base58> form")
	...
	fs.String(BootstrapIDsKey, "", "[Deprecated: use --bootstrap-nodes] Comma separated list of bootstrap peer ids to connect to, each in NodeID-<base58> form")
