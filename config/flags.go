...
func addNodeFlags(fs *pflag.FlagSet) {
	...
	fs.String(StakingKMSEndpointKey, "", "Lux KMS ZAP peer (host:port) for native staking-identity custody. When set, the staking identity is resolved from KMS over an ML-DSA-65-authenticated envelope, superseding every file/content source and failing closed on any error")
	fs.String(StakingKMSEnvKey, "", fmt.Sprintf("KMS environment field for %s (dev | test | main)", StakingKMSEndpointKey))
	fs.String(StakingKMSMnemonicPathKey, "", "DERIVE mode: KMS path of the deploy mnemonic (e.g. providers/lux/deploy-mnemonic). Node derives its identity deterministically at --staking-kms-validator-index. Mutually exclusive with --staking-kms-identity-path")
	fs.Uint(StakingKMSValidatorIndexKey, 0, "DERIVE mode: BIP-44 validator index this node derives at")
	fs.String(StakingKMSIdentityPathKey, "", "CUSTODY mode: KMS path of this node's stored staking-identity blob. Mutually exclusive with --staking-kms-mnemonic-path")
	fs.Bool(StakingKMSSaveKey, false, "CUSTODY mode: mint a fresh identity and save it to KMS when none is stored (requires operator authority on the KMS principal)")
	fs.Bool(StakingKMSStrictPQKey, false, "Also resolve/install the FIPS 204 ML-DSA-65 + FIPS 203 ML-KEM-768 materials so the node runs a strict-PQ NodeID. MANDATORY in DERIVE mode (classical DERIVE churns the NodeID every boot)")
	fs.String(StakingKMSBootstrapMnemonicKey, "", "BIP-39 phrase seeding the ML-DSA-65 envelope-auth identity used to sign KMS requests. Empty dials anonymously (dev/loopback or network-boundary-trusted KMS only)")
	fs.String(StakingKMSNodeLabelKey, "", "CUSTODY mode: operator-supplied node label bound into the stored staking-identity blob and re-checked on load. A node whose label does not match the stored blob refuses to boot (equivocation tripwire for two nodes sharing one identity path)")
	fs.Bool(StakingKMSAllowEnvMnemonicKey, false, "DEV-ONLY: permit the MNEMONIC environment variable to supersede a configured --staking-kms-endpoint in DERIVE mode. With a real (non-loopback) endpoint and this false, the configured endpoint forces the authenticated KMS read and a stray MNEMONIC env is ignored")
