...
See [here](https://build.lux.network/docs/api-reference/admin-api) for more information.

#### `--api-health-enabled` (boolean)

If set to `false`, this node will not expose the Health API. Defaults to `true`. See
[here](https://build.lux.network/docs/api-reference/health-api) for more information.
...
[here](https://build.lux.network/docs/api-reference/index-api) for more information.

#### `--api-info-enabled` (boolean)

If set to `false`, this node will not expose the Info API. Defaults to `true`. See
[here](https://build.lux.network/docs/api-reference/info-api) for more information.

#### `--api-metrics-enabled` (boolean)

If set to `false`, this node will not expose the Metrics API. Defaults to
`true`. See [here](https://build.lux.network/docs/api-reference/metrics-api) for more information.
...
categories: network upgrades and optional chain configurations. Lux
...
[here](https://build.lux.network/docs/nodes/chain-configs). Defaults to `$HOME/.luxd/configs/chains`.
If this flag is not provided and the default directory does not exist,
Lux will not exit since custom configs are optional. However, if the
flag is set, the specified folder must exist, or Lux will exit with an
...
found in a separate [chain config flags](https://build.lux.network/docs/nodes/chain-configs) document.

Full reference for `subnet-evm` upgrade configuration can be found in a separate
[Customize a Subnet](https://build.lux.network/docs/lux-l1s/upgrade/customize-lux-l1) document.
...
Chain specific data directory. Defaults to `$HOME/.luxd/chainData`.
...
:::tip
[Install Script](https://build.lux.network/docs/tooling/lux-go-installer) creates the
node config file at `~/.luxd/configs/node.json`. No default file is
created if [Lux is built from source](https://build.lux.network/docs/nodes/run-a-node/from-source), you
...
Defaults to `$HOME/.luxd`.

## Database

##### `--db-dir` (string, file path)

Specifies the directory to which the database is persisted. Defaults to `"$HOME/.luxd/db"`.
...
Specifies the directory in which system logs are kept. Defaults to `"$HOME/.luxd/logs"`.
...
Lux supports collecting and exporting [OpenTelemetry](https://opentelemetry.io/) traces.
...
`$HOME/.luxd/staking/staker.crt`. This flag is ignored if
...
`$HOME/.luxd/staking/staker.key`. This flag is ignored if
...
`{subnetID}.json` config file under `--subnet-config-dir`. Lux loads
...
separate [Subnet Configs](https://build.lux.network/docs/nodes/configure/lux-l1-configs) document.
...
the specified folder must exist, or Lux will exit with an error. This
...
`$HOME/.luxd/configs/subnets/p4jUwqZsA2LuSftroCd3zb4ytH8W99oXKuKVZdsty7eQ3rXD6.json`.
...
at this directory. Defaults to the `$HOME/.luxd/profiles/`.
...
Sets the directory for [VM plugins](https://build.lux.network/docs/virtual-machines). The default value is `$HOME/.luxd/plugins`.
...
`~/.luxd/configs/vms/aliases.json`. This flag is ignored if
