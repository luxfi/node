...
type Config struct {
	...
	// Chains is the operator's consent list: the chains this node agrees to
	// run, named by letter or alias ("D", "dex", "dexvm"). Some
	// primary-network chains cost an operator something that validating the
	// primary network did not ask for — co-location with a matcher, an HSM, a
	// GPU, a custody role — so the node runs them only when they are named
	// here, even under --track-all-chains.
	//
	// Enforcement is real, not advisory: this list resolves to chain IDs and
	// flows into chains.ManagerConfig.Consented, where
	// authorizeChainActivation declines any consent-requiring chain the
	// operator did not name.
	//
	// It is NECESSARY but not always SUFFICIENT. Where a chain is also
	// restricted it composes AND-wise with the entitlement: consent says what
	// the operator WANTS, the M-Chain attestation says what they are ENTITLED
	// to, and only the attestation lives somewhere the operator cannot
	// rewrite.
