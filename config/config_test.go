...
func TestParseCriticalChains(t *testing.T) {
	tests := []struct {
		name     string
		input    string
		expected []string
		...
		{
			name:     "P always included even if omitted",
			input:    "Q,C",
			expected: []string{"P", "Q", "C"},
		},
		{
			name:     "empty string still gets P",
			input:    "",
			expected: []string{"P"},
		},
		{
		...
		{
			name:     "all chain aliases",
			input:    "P,Q,C,X,D",
			expected: []string{"P", "Q", "C", "X", "D"},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			v := viper.New()
			v.Set(CriticalChainsKey, tt.input)
			result := parseCriticalChains(v)
