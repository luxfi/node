// github.com/luxfi/node — the node, in three languages.
//
// This module is the Go one, and it is the node itself rather than a host that
// fetches one: a repo that reaches outside itself for the thing it claims to be
// has not replaced anything. Rust and C++ live beside it in this same repo and
// are measured against it by the differential.
//
// Nothing here depends on ava-labs. That is checked at build time, not assumed.
...
require (
	...
	github.com/luxfi/dex v1.14.38 // indirect
