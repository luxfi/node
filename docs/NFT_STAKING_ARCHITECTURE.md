...
This document defines the NFT staking mechanism for Lux validators, where NFTs containing LUX can be staked instead of direct LUX staking. NFT ownership or 100,000 LUX stake allows running a Lux mainnet validator and all subnet validators in the Lux ecosystem. Bridge operators require 1,000,000 LUX stake (NFT or direct) and are selected from the top 50% of validators by stake. The bridge operator set can expand or contract daily via epochs, thanks to Corona's flexible key management.
...
type NFTStakeData struct {
    ...
    // Corona signature for ownership
    OwnerSig       *corona.Signature `serialize:"true" json:"ownerSignature"`
}
...
type BridgeValidatorSelector struct {
    ...
    // Epoch management for Corona key stability
...
func (s *BridgeValidatorSelector) calculateNewValidatorSet() []*BridgeValidator {
    ...
    // Select top 50% (flexible size thanks to Corona)
...
// Create new epoch with Corona key management
func (s *BridgeValidatorSelector) createNewEpoch(
    ...
    // Handle Corona key updates based on transition
    switch transition {
    case ValidatorAdded, ValidatorRemoved, ValidatorReplaced:
        // Need new Corona DKG ceremony
        keys, err := s.performCoronaDKG(newValidatorSet)
        if err != nil {
            return err
        }
        epoch.CoronaKeys = keys
        
    case KeyRotation:
        // Scheduled key rotation
        keys, err := s.rotateCoronaKeys(s.currentEpoch.CoronaKeys, newValidatorSet)
        if err != nil {
            return err
        }
        epoch.CoronaKeys = keys
...
func (m *KeyReshareManager) InitiateKeyReshare(
    ...
    case PROTOCOL_CORONA:
        return m.reshareCorona(ceremony)
