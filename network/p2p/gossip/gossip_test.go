...
func TestGossiperShutdown(*testing.T) {
	...
	go func() {
		Every(ctx, log.NoLog{}, gossiper, time.Second)
		wg.Done()
	}()
...
func TestGossiperGossip(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			responseNetwork, err := p2p.NewNetwork(log.NoLog{}, responseSender, prometheus.NewRegistry(), "")
			require.NoError(err)
			...
			requestNetwork, err := p2p.NewNetwork(log.NoLog{}, requestSender, prometheus.NewRegistry(), "")
			require.NoError(err)
...
func TestEvery(*testing.T) {
	...
	go Every(ctx, log.NoLog{}, gossiper, time.Millisecond)
	<-ctx.Done()
}
