...
func TestMessageRouting(t *testing.T) {
	...
	network, err := NewNetwork(nil, sender, luxmetrics.NewNoOpMetrics("test").Registry(), "")
	require.NoError(err)
...
func TestResponseForUnrequestedRequest(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.ErrorIs(err, ErrUnrequestedResponse)
			err = network.CrossChainAppRequestFailed(ctx, ids.Empty, 0, core.ErrTimeout)
...
func TestAppRequestAnyNodeSelection(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			n, err := NewNetwork(nil, sender, luxmetrics.NewNoOpMetrics("test").Registry(), "")
			require.NoError(err)
...
func TestMultipleClients(t *testing.T) {
	...
	n, err := NewNetwork(nil, &core.SenderTest{}, luxmetrics.NewNoOpMetrics("test").Registry(), "")
	require.NoError(err)
	_ = n.NewClient(0)
	_ = n.NewClient(0)
}
