...
import (
	...
	"github.com/luxfi/node/chain/networking/tracker/trackermock"
...
func TestNewSystemThrottler(t *testing.T) {
	...
	reg := luxmetrics.NewNoOpMetrics("test").Registry()
...
func TestSystemThrottler(t *testing.T) {
	...
	targeter := trackermock.NewTargeter(ctrl)
	throttler, err := NewSystemThrottler("", luxmetrics.NewNoOpMetrics("test").Registry(), config, mockTracker, targeter)
	require.NoError(err)
