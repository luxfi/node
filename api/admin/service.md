...
<Callout title="Note">
The Admin API is disabled by default for security reasons. To run a node with the Admin API enabled, use [`config flag --api-admin-enabled=true`](https://docs.lux.network/docs/nodes/configure/configs-flags#--api-admin-enabled-boolean).
...
This API uses the `json 2.0` RPC format. For details, see [here](https://docs.lux.network/docs/api-reference/guides/issuing-api-calls).
...
Dynamically loads any virtual machines installed on the node as plugins. See [here](https://docs.lux.network/docs/virtual-machines#installing-a-vm) for more information on how to install a virtual machine on a node.
