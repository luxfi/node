...
echo "Building minimal luxd binary with stubs..."

# Create a minimal main that just starts the node
...
go build -o build/luxd main_minimal.go

if [ -f build/luxd ]; then
    echo "✅ Minimal build successful!"
    echo "Binary: $(pwd)/build/luxd"
    chmod +x build/luxd
    ./build/luxd --version
else
    echo "❌ Build failed"
