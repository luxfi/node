...
const (
	LuxPathEnvName = "LUXD_PATH"

	defaultNodeInitTimeout = 10 * time.Second
)

var (
	LuxPluginDirEnvName = config.EnvVarName(config.EnvPrefix, config.PluginDirKey)
...
func (p *ProcessRuntime) Start(ctx context.Context) error {
	...
	cmd := exec.Command(runtimeConfig.LuxPath, "--config-file", p.node.GetFlagsPath()) // #nosec G204
