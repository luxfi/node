...
// Collects the configuration for running a temporary luxd network
type Network struct {
	// Uniquely identifies the temporary network for metrics
	// collection. Distinct from luxd's concept of network ID
...
func StopNetwork(ctx context.Context, log luxlog.Logger, dir string) error {
...
func RestartNetwork(ctx context.Context, log luxlog.Logger, dir string) error {
	network, err := ReadNetwork(ctx, log, dir)
	if err != nil {
		return err
	}
...
func ReadNetwork(ctx context.Context, log luxlog.Logger, dir string) (*Network, error) {
...
func (n *Network) EnsureDefaultConfig(log luxlog.Logger) error {
...
func (n *Network) StartNodes(ctx context.Context, log luxlog.Logger, nodesToStart ...*Node) error {
...
func (n *Network) Bootstrap(ctx context.Context, log luxlog.Logger) error {
	if len(n.Subnets) == 0 {
...
func WaitForHealthyNodes(ctx context.Context, log luxlog.Logger, nodes []*Node) error {
	for _, node := range nodes {
...
func (n *Network) CreateSubnets(ctx context.Context, log luxlog.Logger, apiURI string, restartRequired bool) error {
...
//
// For consumption outside of luxd. Needs to be kept exported.
...
// luxd binary - mainnet, testnet and local), the value of the
...
func waitForHealthy(ctx context.Context, log luxlog.Logger, nodes []*Node) error {
...
func checkVMBinaries(log luxlog.Logger, subnets []*Subnet, config *ProcessRuntimeConfig) error {
	if len(subnets) == 0 {
	...
	luxdRPCVersion, err := getRPCVersion(log, config.LuxPath, "--version-json")
	if err != nil {
		log.Warn("unable to check rpcchainvm version for luxd", zap.Error(err))
		return nil
	}
	...
	for _, subnet := range subnets {
		for _, chain := range subnet.Chains {
			...
			if len(chain.VersionArgs) == 0 || luxdRPCVersion == invalidRPCVersion {
				// Not possible to check the rpcchainvm version
				continue
			}

			// Check that the VM's rpcchainvm version matches luxd's version
			...
			} else if luxdRPCVersion != vmRPCVersion {
				log.Error("unexpected rpcchainvm version for VM binary",
					zap.String("subnet", subnet.Name),
					zap.String("luxdPath", config.LuxPath),
					zap.Uint64("luxdRPCVersion", luxdRPCVersion),
					zap.String("vmPath", vmPath),
	...
	if incompatibleChains {
		return errors.New("the rpcchainvm version of the VMs for one or more chains may not be compatible with the specified luxd binary")
	}
	return nil
}
...
func getRPCVersion(log luxlog.Logger, command string, versionArgs ...string) (uint64, error) {
