...
| flagsmap.go                 | FlagsMap       | Simplifies configuration of node flags                          |
...
$ ./bin/tmpnetctl start-network --node-path=/path/to/node
...
            ...
            Chains: []*tmpnet.Chain{
                {
                    ...
                    VersionArgs: "version-json", // (Optional) Arguments that prompt the VM binary to output version details in json format.
                                                 ...
                                                 // The version will be checked against the version reported by the configured node
...
- name: Run e2e tests
  ...
  # A qualified path is required for use outside of node
