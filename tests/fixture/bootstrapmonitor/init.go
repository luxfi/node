...
func InitBootstrapTest(log log.Logger, namespace string, podName string, nodeContainerName string, dataDir string) error {
	...
	if strings.HasSuffix(testConfig.Image, ":latest") {
		log.Info("Determining image id for image", logfields.UserString("image", testConfig.Image))
	...
	if testDetailsBytes, err := os.ReadFile(testDetailsPath); errors.Is(err, os.ErrNotExist) {
		log.Info("Test details file does not exist", logfields.UserString("path", testDetailsPath))
	} else if err != nil {
	...
	if testDetails.Image == testConfig.Image {
		...
		log.Info(BootstrapResumingMessage, logfields.Reflect("testConfig", testConfig))
		return nil
	...
	log.Info("Removing node directory", logfields.UserString("path", nodeDataDir))
	...
	log.Info("Writing test details to file",
		logfields.Reflect("testDetails", testDetails),
		logfields.UserString("path", testDetailsPath),
	)
	...
	log.Info(BootstrapStartingMessage,
		...
		logfields.Time("startTime", testDetails.StartTime),
	)
