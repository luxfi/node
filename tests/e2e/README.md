...
```bash
./scripts/build.sh        # Builds node for use in deploying a test network
./scripts/build_xsvm.sh   # Builds xsvm for use in deploying a test network with a subnet
./bin/ginkgo -v ./tests/e2e -- --node-path=$PWD/build/node # Note that the path given for --node-path must be an absolute and not a relative path.
```
...
without having to specify the `--node-path` or `--plugin-dir`
flags.
...
```bash
./bin/ginkgo -v --label-filter=x ./tests/e2e -- --node-path=$PWD/build/node
```
...
```bash
./bin/gingko -v ./tests/e2e -- --node-path=/path/to/node --reuse-network
```
...
When iterating on a change to node and/or a VM, it may be
