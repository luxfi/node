...
func (te *TeleportEngine) processNFTTransfer(ctx context.Context, transfer *TeleportTransfer) (*TeleportTransfer, error) {
	...
	default:
		...
		return te.processGenericNFTTransfer(ctx, transfer)
	}
}
...
func (te *TeleportEngine) processXChainToCChainNFT(ctx context.Context, transfer *TeleportTransfer) (*TeleportTransfer, error) {
	...
	mintTx, err := te.cChainClient.MintNFT(
		transfer.Asset.ContractAddress,
		transfer.Recipient,
		transfer.Asset.TokenID,
		transfer.Asset.Metadata,
		zkProof,
	)
	if err != nil {
		te.updateTransferStatus(transfer.ID, TransferStatusFailed)
		return nil, fmt.Errorf("failed to mint NFT on C-Chain: %w", err)
	}
	
	// Step 4: Update asset registry
	te.assetRegistry.UpdateAssetLocation(transfer.Asset.ID, transfer.DestChain)
...
func (te *TeleportEngine) processCChainToXChainNFT(ctx context.Context, transfer *TeleportTransfer) (*TeleportTransfer, error) {
	...
	burnTx, err := te.cChainClient.BurnNFT(
		transfer.Asset.ContractAddress,
		transfer.Asset.TokenID,
		transfer.Sender,
	)
	if err != nil {
		te.updateTransferStatus(transfer.ID, TransferStatusFailed)
		return nil, fmt.Errorf("failed to burn NFT on C-Chain: %w", err)
	}
	...
	mintProof, err := te.xchainSettlement.ProcessIncomingAssets(
		ctx,
...
func (te *TeleportEngine) processNFTToPChain(ctx context.Context, transfer *TeleportTransfer) (*TeleportTransfer, error) {
	...
	case constants.EVMID:
		// Lock on C-Chain
		lockProof, err = te.cChainClient.LockNFT(
			transfer.Asset.ContractAddress,
			transfer.Asset.TokenID,
			transfer.Sender,
		)
	...
	validatorTx, err := te.pChainClient.AddValidatorWithNFT(
		validatorNFT.ValidatorNodeID,
		validatorNFT.StakeAmount,
		validatorNFT.StakeStartTime,
		validatorNFT.StakeEndTime,
		validatorNFT.DelegationFee,
		transfer.Asset.ID,
		zkProof,
	)
	if err != nil {
	...
	te.assetRegistry.UpdateAssetLocation(transfer.Asset.ID, constants.PlatformVMID)
	te.nftHandler.ActivateValidatorNFT(transfer.Asset.ID, validatorTx.ID())
...
func (te *TeleportEngine) processFungibleTransfer(ctx context.Context, transfer *TeleportTransfer) (*TeleportTransfer, error) {
	...
	err = te.executorEngine.ExecuteTransfer(ctx, transfer, settlementProof)
	if err != nil {
		te.updateTransferStatus(transfer.ID, TransferStatusFailed)
...
func (te *TeleportEngine) processPendingIntents() {
	intents := te.intentPool.GetPendingIntents(100)
