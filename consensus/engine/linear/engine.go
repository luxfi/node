...
import (
	...
	"github.com/luxfi/node/consensus/linear/poll"
...
func (e *Engine) Start(ctx context.Context, startReqID uint32) error {
	...
	if oracleBlk, ok := lastAccepted.(linear.OracleBlock); ok {
		options, err := oracleBlk.Options(ctx)
		switch {
		case err == linear.ErrNotOracle:
...
func (e *Engine) deliver(
	...
	if blk, ok := blk.(linear.OracleBlock); ok {
		options, err := blk.Options(ctx)
		if err != linear.ErrNotOracle {
			if err != nil {
				return err
			}
