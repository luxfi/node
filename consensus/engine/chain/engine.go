...
func New(config Config) (*Engine, error) {
	...
	return &Engine{
		...
		AncestorsHandler:            core.NewNoOpAncestorsHandler(config.Ctx.Log),
...
func (e *Engine) Notify(ctx context.Context, msg core.Message) error {
	switch msg {
	case core.PendingTxs:
		// the pending txs message means we should attempt to build a block.
		e.pendingBuildBlocks++
		return e.executeDeferredWork(ctx)
	case core.StateSyncDone:
...
func (e *Engine) Start(ctx context.Context, startReqID uint32) error {
	...
	// Check for imported blockchain data override
	if importedBlockID := os.Getenv("LUX_IMPORTED_BLOCK_ID"); importedBlockID != "" {
		e.Ctx.Log.Info("using imported blockchain data",
			zap.String("blockID", importedBlockID),
		)
		
		// Parse the imported block ID
		blockIDBytes, err := hex.DecodeString(importedBlockID)
		if err != nil {
			return fmt.Errorf("failed to decode imported block ID: %w", err)
		}
		
		lastAcceptedID, err := ids.ToID(blockIDBytes)
		if err != nil {
			return fmt.Errorf("failed to convert imported block ID: %w", err)
		}
		
		// Get the imported block height and timestamp from env vars
		importedHeight := uint64(1082780) // Default to our known height
		if heightStr := os.Getenv("LUX_IMPORTED_HEIGHT"); heightStr != "" {
			if parsedHeight, err := strconv.ParseUint(heightStr, 10, 64); err == nil {
				importedHeight = parsedHeight
			}
		}
		
		importedTime := time.Now() // Default to current time
		if timeStr := os.Getenv("LUX_IMPORTED_TIMESTAMP"); timeStr != "" {
			if parsedTime, err := strconv.ParseInt(timeStr, 10, 64); err == nil {
				importedTime = time.Unix(parsedTime, 0)
			}
		}
		
		// Initialize consensus with imported data
		if err := e.Consensus.Initialize(e.Ctx, e.Params, lastAcceptedID, importedHeight, importedTime); err != nil {
			return fmt.Errorf("failed to initialize consensus with imported data: %w", err)
		}
		
		// Set VM preference to imported block
		if err := e.VM.SetPreference(ctx, lastAcceptedID); err != nil {
			e.Ctx.Log.Warn("failed to set VM preference to imported block",
				zap.Error(err),
			)
		}
		
		e.Ctx.Log.Info("consensus initialized with imported blockchain",
			zap.Stringer("blockID", lastAcceptedID),
			zap.Uint64("height", importedHeight),
			zap.Time("timestamp", importedTime),
		)
		
		return nil
	}
	
	// Original code path for normal startup
...
func (e *Engine) sendChits(ctx context.Context, nodeID ids.NodeID, requestID uint32, requestedHeight uint64) {
	...
	if e.Ctx.StateSyncing.Get() || e.Config.PartialSync {
		...
		if err != nil {
			...
			// That means that this code path is actually quite core.
