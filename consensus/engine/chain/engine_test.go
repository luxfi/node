...
func MakeGetBlockF(blks ...[]*chaintest.Block) func(context.Context, ids.ID) (consensuschain.Block, error) {
	return func(_ context.Context, blkID ids.ID) (consensuschain.Block, error) {
...
func MakeParseBlockF(blks ...[]*chaintest.Block) func(context.Context, []byte) (consensuschain.Block, error) {
	return func(_ context.Context, blkBytes []byte) (consensuschain.Block, error) {
		for _, blkSet := range blks {
			for _, blk := range blkSet {
...
func setup(t *testing.T, config Config) (ids.NodeID, validators.Manager, *enginetest.Sender, *blocktest.VM, *Engine) {
	...
	consensusGetHandler, err := getter.New(
		vm,
		sender,
		config.Ctx.Log,
		time.Second,
	...
	require.NoError(err)
	config.AllGetsServer = consensusGetHandler
	...
	vm.GetBlockF = func(_ context.Context, blkID ids.ID) (consensuschain.Block, error) {
		switch blkID {
...
func TestEngineDropsAttemptToIssueBlockAfterFailedRequest(t *testing.T) {
	...
	vm.ParseBlockF = func(_ context.Context, b []byte) (consensuschain.Block, error) {
	...
	vm.ParseBlockF = func(context.Context, []byte) (consensuschain.Block, error) {
...
func TestEngineMultipleQuery(t *testing.T) {
	...
	vm.GetBlockF = func(_ context.Context, id ids.ID) (consensuschain.Block, error) {
		switch id {
		case chaintest.GenesisID:
			return chaintest.Genesis, nil
		case blk0.ID():
...
func TestEngineBuildBlock(t *testing.T) {
	...
	vm.BuildBlockF = func(context.Context) (consensuschain.Block, error) {
		return blk, nil
	}
	require.NoError(te.Notify(context.Background(), core.PendingTxs))
...
func TestEngineBlockingChitResponse(t *testing.T) {
	...
	vm.ParseBlockF = func(_ context.Context, blkBytes []byte) (consensuschain.Block, error) {
		switch {
...
func TestEngineDropRejectedBlockOnReceipt(t *testing.T) {
	...
	// Issue [acceptedBlk] to the core. This
...
func TestEngineBubbleVotesThroughInvalidBlock(t *testing.T) {
	...
	// This engine receives a Gossip message for [blk2] which was "unknown" in this core.
...
func TestEngineVoteStallRegression(t *testing.T) {
	...
	var getBlock3Request *core.Request
	sender.SendGetF = func(_ context.Context, nodeID ids.NodeID, requestID uint32, blkID ids.ID) {
		require.Nil(getBlock3Request)
		require.Equal(nodeID2, nodeID)
		getBlock3Request = &core.Request{
			NodeID:    nodeID,
			RequestID: requestID,
		}
...
func TestGetProcessingAncestor(t *testing.T) {
	var (
		...
		emptyNonVerifiedCache             = &cache.Empty[ids.ID, consensuschain.Block]{}
		nonVerifiedCacheWithUnissuedBlock = lru.NewCache[ids.ID, consensuschain.Block](1)
		nonVerifiedCacheWithDecidedBlock  = lru.NewCache[ids.ID, consensuschain.Block](1)
	)
	...
	tests := []struct {
		...
		nonVerifiedCache cache.Cacher[ids.ID, consensuschain.Block]
		initialVote      ids.ID
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			var (
				...
				consensus = &consensuschain.Topological{Factory: factories.ConsensusflakeFactory}
			)
			...
			metrics, err := newMetrics(metrics.NewTestRegistry())
			require.NoError(err)
...
func TestShouldIssueBlock(t *testing.T) {
	...
	c := &consensuschain.Topological{Factory: factories.ConsensusflakeFactory}
	...
	engine := &Engine{
		...
		pending: map[ids.ID]consensuschain.Block{
...
func TestEngineAcceptedHeight(t *testing.T) {
	...
	vm.GetBlockF = func(context.Context, ids.ID) (consensuschain.Block, error) {
		return blk1, nil
	}
