...
func TestPushAndExecute(t *testing.T) {
	...
	count, err := jobs.ExecuteAll(context.Background(), consensustest.ConsensusContext(ctx), &core.Halter{}, false)
	require.NoError(err)
...
func TestHandleJobWithMissingDependencyOnRunnableStack(t *testing.T) {
	...
	ctx := consensustest.Context(t, consensustest.CChainID)
	_, err = jobs.ExecuteAll(context.Background(), consensustest.ConsensusContext(ctx), &core.Halter{}, false)
