...
func (b *Bootstrapper) Ancestors(ctx context.Context, nodeID ids.NodeID, requestID uint32, vtxs [][]byte) error {
	...
	var (
		numBytes          = len(vtxs[0])
		verticesToProcess = make([]graph.Vertex, 1, len(vtxs))
	)
...
func (b *Bootstrapper) process(ctx context.Context, vtxs ...graph.Vertex) error {
	...
	toProcess := heap.NewMap[ids.ID, graph.Vertex](vertexLess)
...
func (b *Bootstrapper) startSyncing(ctx context.Context, acceptedContainerIDs []ids.ID) error {
	...
	toProcess := make([]graph.Vertex, 0, len(pendingContainerIDs))
...
func vertexLess(i, j graph.Vertex) bool {
	if !i.Status().Fetched() {
		return true
	}
	if !j.Status().Fetched() {
