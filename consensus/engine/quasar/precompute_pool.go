...
// PrecomputePool manages a pool of precomputed Corona signatures
...
func NewPrecomputePool(targetSize int) *PrecomputePool {
	return &PrecomputePool{
		pool:       make([]corona.Precomp, 0, targetSize),
...
func (p *PrecomputePool) Get() (corona.Precomp, error) {
	p.mu.Lock()
	defer p.mu.Unlock()
...
func (p *PrecomputePool) precomputeWorker() {
	...
	for {
		...
		case <-ticker.C:
			...
			if needMore && sk != nil {
				if precomp, err := corona.Precompute(sk); err == nil {
					p.mu.Lock()
...
func (p *PrecomputePool) fillPool() error {
	...
	for len(p.pool) < p.targetSize {
		precomp, err := corona.Precompute(p.sk)
		if err != nil {
			return err
		}
		p.pool = append(p.pool, precomp)
