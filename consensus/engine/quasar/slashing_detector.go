...
// RecordCoronaShare records a Corona signature share
func (sd *SlashingDetector) RecordCoronaShare(height uint64, nodeID ids.NodeID, blockID ids.ID, share []byte) *SlashingEvent {
	sd.mu.Lock()
	defer sd.mu.Unlock()
...
func (sd *SlashingDetector) Analyze(block *PendingBlock, cert *DualCertificate) *SlashingEvent {
	...
	// Check for validators who signed BLS but not Corona
	for nodeID := range blsSigners {
		if !rtSigners[nodeID] {
			// Mark as missing Corona
...
// MissingCoronaEvidence proves a validator signed BLS but not Corona
type MissingCoronaEvidence struct {
	Height       uint64
	BlockID      ids.ID
	BLSSignature []byte
}
