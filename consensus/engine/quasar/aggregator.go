...
var (
	...
	ErrCoronaAggregationFailed = errors.New("Corona aggregation failed")
	ErrDualCertMismatch      = errors.New("BLS and Corona certificates do not match")
	ErrQuasarTimeout         = errors.New("Quasar finality timeout")
)

// DualCertificate represents a finalized block with both BLS and Corona signatures
...
// Aggregator manages parallel BLS and Corona signature aggregation
type Aggregator struct {
	...
	coronaShares  map[uint64]map[ids.NodeID]corona.Share
...
// ValidatorKeys holds both BLS and Corona public keys for a validator
...
func NewAggregator(nodeID ids.NodeID, threshold int, validators map[ids.NodeID]*ValidatorKeys) *Aggregator {
	return &Aggregator{
		...
		coronaShares: make(map[uint64]map[ids.NodeID]corona.Share),
...
// AddCoronaShare adds a Corona signature share for aggregation
func (a *Aggregator) AddCoronaShare(height uint64, nodeID ids.NodeID, share corona.Share) error {
	a.mu.Lock()
	defer a.mu.Unlock()
	
	if _, exists := a.coronaShares[height]; !exists {
		a.coronaShares[height] = make(map[ids.NodeID]corona.Share)
	}
	
	a.coronaShares[height][nodeID] = share
...
func (a *Aggregator) tryAggregate(height uint64) error {
	...
	rtCount := len(a.coronaShares[height])
	...
	// Run BLS and Corona aggregation in parallel
	start := time.Now()
	...
	// Corona aggregation goroutine
	wg.Add(1)
	go func() {
		defer wg.Done()
		rtCert, rtErr = a.aggregateCorona(height, signerIDs)
		if rtErr == nil {
			a.metrics.CoronaAggregations++
		}
	}()
	...
	if rtErr != nil {
		return fmt.Errorf("Corona aggregation failed: %w", rtErr)
	}
	...
	delete(a.coronaShares, height)
	
	return nil
}

// getMatchingSigners returns validators who provided both BLS and Corona signatures
func (a *Aggregator) getMatchingSigners(height uint64) []ids.NodeID {
	var signers []ids.NodeID
	
	blsSigners := a.blsShares[height]
	rtSigners := a.coronaShares[height]
...
// aggregateCorona performs Corona threshold signature aggregation
func (a *Aggregator) aggregateCorona(height uint64, signerIDs []ids.NodeID) ([]byte, error) {
	shares := a.coronaShares[height]
	
	// Collect shares for aggregation
	rtShares := make([]corona.Share, 0, len(signerIDs))
	for _, nodeID := range signerIDs {
	...
	cert, err := corona.Aggregate(rtShares)
	if err != nil {
		return nil, fmt.Errorf("%w: %v", ErrCoronaAggregationFailed, err)
	}
...
// getCurrentEpoch returns the current Corona key epoch
