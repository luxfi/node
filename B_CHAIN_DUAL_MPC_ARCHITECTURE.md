# B-Chain Dual MPC Architecture: CGGMP21 + Corona

## Overview

B-Chain implements global network-level consensus with dual MPC protocol support: CGGMP21 for ECDSA compatibility and Corona for post-quantum security, maintaining full compatibility with the existing bridge API.
...
const (
    ...
    PROTOCOL_CORONA                      // Post-quantum lattice signatures
...
type BridgeRequest struct {
    ...
    Protocol    string          `json:"protocol,omitempty"`  // "ecdsa", "corona", "auto"
}
...
func (api *BridgeAPIAdapter) HandleBridgeRequest(req *BridgeRequest) (*BridgeResponse, error) {
    ...
    case PROTOCOL_CORONA:
        signature, err = api.mpcManager.SignWithCorona(bridgeTx)
...
func (mpc *MPCConsensusEngine) ProposeMPCOperation(op MPCOperation) error {
    ...
    case PROTOCOL_CORONA:
        return mpc.initiateCoronaCeremony(ceremony, required)
    }
}
```
...
### 3.2 Corona Native Implementation
```go
// vms/bvm/mpc/corona/native.go
package corona

type NativeCoronaMPC struct {
    params      *CoronaParams
    validators  map[ids.NodeID]*CoronaValidator
}

// Threshold key generation for Corona
func (r *NativeCoronaMPC) DistributedKeyGen() (*CoronaDKGResult, error) {
...
type BridgeTransaction struct {
    ...
    CoronaSig  *corona.Signature  `json:"coronaSignature,omitempty"`
...
│     └── Determine required protocol (ECDSA/Corona)        │
│                                                              │
│  3. MPC Ceremony Initiated                                  │
│     ├── CGGMP21: Use presigned data for speed              │
│     └── Corona: Full threshold signing                    │
│                                                              │
...
│     └── Corona: Combine lattice-based shares             │
│                                                              │
...
func (api *BridgeAPI) GetMPCStatus() (*MPCStatus, error) {
    return &MPCStatus{
        ...
        Corona: &ProtocolStatus{
            KeysGenerated:   api.metrics.CoronaStats.KeysGenerated,
            AverageSignTime: api.metrics.CoronaStats.AvgSignTime(),
        },
...
- **Corona**: 256-bit post-quantum security
...
This architecture ensures B-Chain maintains full compatibility with the existing bridge API while upgrading to CGGMP21 and adding Corona support for quantum safety.