...
type BridgeWallet interface {
	...
	// VerifyBridgeAttestation verifies a bridge attestation with Corona signatures
...
type SignedTransaction interface {
	...
	// GetCoronaSignature returns the Corona signature if present
	GetCoronaSignature() []byte
}
...
type BridgeAttestation struct {
	...
	CoronaSig      []byte           // Aggregated Corona signature
}
...
type KeyManager interface {
	...
	// GetCoronaShare returns the Corona key share if available
	GetCoronaShare() []byte
	
	// Sign signs a message
	Sign(message []byte) ([]byte, error)
	
	// SignWithCorona signs with Corona if available
	SignWithCorona(message []byte) ([]byte, error)
}
