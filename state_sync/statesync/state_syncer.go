...
func NewStateSyncer(config *StateSyncerConfig) (*stateSync, error) {
	ss := &stateSync{
		...
		segments:         make(chan statesyncclient.LeafSyncTask, defaultNumThreads*numStorageTrieSegments),
		mainTrieDone:     make(chan struct{}),
		storageTriesDone: make(chan struct{}),
		done:             make(chan error, 1),
	}
	ss.syncer = statesyncclient.NewCallbackLeafSyncer(config.Client, ss.segments, config.RequestSize)
